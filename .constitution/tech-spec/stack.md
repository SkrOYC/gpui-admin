...
All versions below verified against crates.io / upstream source on 2026-07-09.
...
| :--- | :--- | :--- |
| `gpui` | `=0.2.2` | Reactive runtime: entities, observation, tasks, windows (R2 observation surface, R5 windows). crates.io publication verified; pre-1.0 with breaking-change warning → exact pin. |
| `gpui_platform` | `=0.2.2` (matching gpui) | Platform backends (x11/wayland/macOS). Required alongside `gpui` per upstream layout. |
...
1. **Substrate family (`gpui`, `gpui_platform`, `gpui-component`, `gpui_http_client`): exact pins (`=x.y.z`).** Upgrades are deliberate, reviewed acts: one PR, pairing all four, with the tracer-bullet seams re-exercised. Never floated, never auto-bumped (RSK-02).
