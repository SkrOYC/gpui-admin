...
- **Licensing/packaging:** `MIT OR Apache-2.0`; crates.io family `gpui-admin`, `gpui-admin-core`, `gpui-admin-macros`, `gpui-admin-cli`, `gpui-admin-ui`, `gpui-admin-conformance`, `gpui-admin-provider-supabase` (first four verified unclaimed 2026-07-09; remaining three checked at reservation time — the reservation ticket stops on any collision).
